...
def test_patch_bay_arrays_and_knobs_have_balanced_layout():
    ...
    assert 'href="style.css?v=20260723-5x5-deploy"' in index_html
...
def test_synth_wash_controls_default_to_seventy_percent():
    ...
    assert '<input id="reverb-slider" type="range" min="0" max="1.5" step="0.01" value="1.05" /><output>70%</output>' in index_html
    assert '<input id="delay-slider" type="range" min="0" max="1" step="0.01" value="0.7" /><output>70%</output>' in index_html
    assert 'from "./tone_engine.js?v=20260723-5x5-deploy"' in main_js
    assert "const REVERB_UI_MAX = 1.5;" in tone_engine_js
    assert "const normalized = clamp((Number.isFinite(amount) ? amount : 0) / REVERB_UI_MAX, 0, 1);" in tone_engine_js
    assert "const shaped = normalized * normalized;" in tone_engine_js
...
def test_synth_trigger_grid_is_more_responsive():
    ...
    assert 'from "./scheduler.js?v=20260723-5x5-deploy"' in main_js
    assert "pluck: [0.5, 4]" in scheduler_js
    assert "bell: [0.75, 7]" in scheduler_js
    assert "pad: [8, 24]" in scheduler_js
    assert "bloom: [5, 18]" in scheduler_js
    assert "drone: [16, 48]" in scheduler_js
    assert '{ name: "still", speed: 0.9, probability: 0.56' in scheduler_js
    assert '{ name: "spark", speed: 0.22, probability: 0.7' in scheduler_js
...
def test_synth_play_and_live_patch_creation_audition_connected_voices():
    ...
    assert "immediateDuration(voice)" in scheduler_js
    assert "this.engine.setVoiceGain(voiceId, 0.55, 0.02);" in scheduler_js
    assert "this.engine.triggerVoice(voiceId, this.midiForVoice(voice), this.immediateDuration(voice));" in scheduler_js
